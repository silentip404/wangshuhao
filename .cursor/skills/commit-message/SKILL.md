...
When generating Git commit messages, you **MUST** read and strictly follow these specifications. You **MUST NOT** reference any other commit message configurations, conventions, or formats (e.g., Angular commit guidelines, or repository commit history).
...
### Changes Scope Detection

When the user does not explicitly specify whether to analyze staged or unstaged changes:

- If only staged changes exist → analyze staged changes
- If only unstaged changes exist → analyze unstaged changes
- If both staged and unstaged changes exist → **MUST** ask the user to specify the scope before proceeding
