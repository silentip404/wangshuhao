...
if ('rules' in typescriptPlugin) {
  // 验证插件 rules 结构，确保运行时安全
  const rulesSchema = z.record(z.string(), z.unknown());
  const parsedRules = rulesSchema.safeParse(typescriptPlugin.rules);
...
const typescriptOverrides = defineConfig([
  ...
  },
  {
    ...
    rules: {
       ...
       * - AI 辅助编程场景下，过度的 readonly 约束会频繁触发类型错误，降低开发效率
       */
       ...
       * - TypeScript 编译器能够进行更精确的控制流分析，识别所有可能的代码路径
       */
       ...
       *
       * @reason
       * - 强制显式类型转换，提高代码的健壮性和可维护性
       * - 防止隐性类型转换导致的 [object Object] 等不符合预期的字符串
       */
